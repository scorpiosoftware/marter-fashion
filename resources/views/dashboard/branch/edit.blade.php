<x-app-layout>
    ...
    <form class="pt-4" action="{{ route('branch.update',$record->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    <select id="section_id" name="section_id"
                        ...
                        @foreach ($sections as $section)
                            <option value="{{ $section->id }}" {{$section->id == $record->section_id ? 'selected' : ''}}>{{ $section->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
